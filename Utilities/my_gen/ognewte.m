function [xn,niter]=ognewte(x0,xp,P0,A,B,RA,CE,Nmax,lam,tol,fast,flag);
%function [xn,niter]=ognewte(x0,xp,P0,A,B,RA,CE,Nmax,lam,tol,fast,flag);

xn=x0;errx=1+tol;meth=2;arm=0;
stdes=0;indcall=1;
...
[fn,df,ddF]=proj_obe(xn,xp,P0,A,B,RA,CE,lam,indcall);
...
while errx(1) > tol(1)
   ...
   if stdes==0
      if fast(1)>0
         ...
         if rcon(1) < tol/10
            ...
            if flag(1)>1;
               disp(['ognewte: hessian correction: cond.num. = ',num2str(rcon)]);
            end
         end
         dk=-(ddF\df);
      elseif fast(1)==0
   ...
   if a(a_ind)==0
      if stdes==1
         errx=0;
         if flag(1)>1;disp('ognewte: no cost reduction along descent direction');end
      else
   ...
   if niter > Nmax(1); 
      errx=0;
      if flag(1)>1;disp('ognewte: max no. of iterations exceeded');end
   end
end
