function [Acd,Bcd,Ccd,Dcd]=m_d_con(P_u,C_y,P_d,W_d,lam,G_w,e_s,flag,slr);
% function [Acd,Bcd,Ccd,Dcd]=m_d_con(P_u,C_y,P_d,W_d,lam,G_w,e_s,flag);
% feedforward controller for a measured disturbance
%   P_u, C_y: Packed systems, u->y plant, y->u controller
%   P_d: dist -> y (packed)
%   W_d: biproper disturbance filter (e.g., uncertainty?)
...
%   e_s: [exp_shift, low-fr cutoff, hi-freq cutoff, 
%         reduction toler, Nehari/H2, Nehari parameter]
% On return
%   [Acd,Bcd,Ccd,Dcd] or
...
[As,Bs,Cs,Ds]=feedbk(Alp,Blp,Clp,Dlp,1);
[Asp,Bsp,Csp,Dsp]=feedbk(Ap,Bp,Cp,Dp,3,Ac,Bc,Cc,Dc);
[Asd,Bsd,Csd,Dsd]=series(Ad,Bd,Cd,Dd,As,Bs,Cs,Ds);
...
%sx=sv3_5(AX,BX,CX,DX,1,frw);
   ...
   sxx=sv3_5(Arx,Brx,Crx,Drx,1,frw);
   loglog(frw,sxx);pause
end
...
Acd0=Acd;Bcd0=Bcd;Ccd0=Ccd;Dcd0=Dcd;
...
if flag>=1
   if ~isempty(W_d)
      ...
      [Acy,Bcy,Ccy,Dcy]=series(Aw,Bw,Cw,Dw,Acd0,Bcd0,Ccd0,Dcd0);
   else
      Acx=Acd;Bcx=Bcd;Ccx=Ccd;Dcx=Dcd;
      Acy=Acd0;Bcy=Bcd0;Ccy=Ccd0;Dcy=Dcd0;
   end
   [Acx,Bcx,Ccx,Dcx]=series(Acx,Bcx,Ccx,Dcx,At2,Bt2,Ct2,Dt2);
   [Acy,Bcy,Ccy,Dcy]=series(Acy,Bcy,Ccy,Dcy,At2,Bt2,Ct2,Dt2);
   sr=sv3_5(At1,Bt1,Ct1,Dt1,1,frw);
   sx=sv3_5(Acx,Bcx,Ccx,Dcx,1,frw);
   sy=sv3_5(Acy,Bcy,Ccy,Dcy,1,frw);
   loglog(frw,sr,'r',frw,sx,'b',frw,sy,'g');
   title('nehari approx: red=target, blue=hinf approx, green=no-reduction')
   pause
end
