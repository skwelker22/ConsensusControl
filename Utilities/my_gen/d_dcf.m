function [S_NL,S_DL,S_NR,S_DR,S_XL,S_YL,S_XR,S_YR]=d_dcf(A,B,C,D,shif,gain);

%function [S_NL,S_DL,S_NR,S_DR,S_XL,S_YL,S_XR,S_YR]=...
%                d_dcf(A,B,C,D,shif,gain);
% Find doubly coprime factorizations for a discrete time system [A,B,C,D]
% P = NR DRinv = DLinv NL
% [ XL  -YL ] [ DR   YR ]
% [-NL   DL ] [ NR   XR ] = I
...
IA=eye(nA,nA);IO=gain*eye(nout,nout);OI=gain*eye(ninp,ninp);
     Lpr=dlqr(A'*(1+shif),C',B*B',IO);Lpr=Lpr';
     Kpr=dlqr(A*(1+shif),B,C'*C,OI);
