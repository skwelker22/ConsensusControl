function [proj,d2,dinf]=hepprjm(theta,A,B,RA,CE,epsst,flag,f_cont);
%
...
thresh=1.e-12;
if nargin<6;flag=0;epsst=0;f_cont=0;end
if nargin<7;flag=0;f_cont=0;end
if nargin<8;f_cont=0;end
   ...
   while dista >= epsst(1)
     ...
     if f_cont(1)~=0 
       index1=max(find(D==max(D)));
         if index1<=mh
           aa=Q(index1,:);bb=B(index1);proj=R*proj;
           proj=proj-aa'*(aa*proj-bb);proj=R\proj;
         else
           index2=index1-mh;
