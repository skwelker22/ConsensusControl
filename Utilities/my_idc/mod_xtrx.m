function [A,B,C,D,F,By,Dy,X0]=mod_xtrx(FF,CC,thx,ninp,noutp,filord,nan_ind);
...
n_ths=(ninp+noutp+1+n_nan)*nmax;
n_thb=(ninp+noutp+1+n_nan)*nmax+ninp;
...
%      q=C(i,nftem+1:nftemi)';
      By=[By;unvector(thx(ninp*n+1:(ninp+noutp)*n,i),n,noutp)];
      B=[B;unvector(thx(1:ninp*n,i),n,ninp)];
        if nc==0
          D(i,:)=(thx((ninp+noutp)*n+1:(ninp+noutp)*n+ninp,i))';
        end
   end
B=B+By*D;A=F+By*C;
   ...
   for i_nan=1:n_nan+1
      ...
      for i=1:noutp
         n=filord(i);
         nan_loc=(ninp+noutp)*n+ninp*(1-nc)+(i_nan-1)*n;
