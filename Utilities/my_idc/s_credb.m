function [Acr,Bcr,Ccr,Dcr,scc]=s_credb(con_file);
%function [Acr,Bcr,Ccr,Dcr]=s_cred(AC_,AP_,AW_,frw,flag,cl_spec);
% Compensator reduction with balancing.
...
if scc ~=0
   if flag >2
     itemp=input('Schur Compensator reduction (0=no)? [1] ');
     if isempty(itemp);itemp=1;end
   else
     itemp=1;
   end
...
W=lyap(Acr',Ccr'*Ccr);[U,S,V]=svd(W);TR=U*sqrt(S);TRi=inv(sqrt(S))*U';
Acr=TR*Acr*TRi;Bcr=TR*Bcr;Ccr=Ccr*TRi;
