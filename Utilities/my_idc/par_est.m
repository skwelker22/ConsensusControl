function [thx,err,nan_ind]=par_est(ninp,noutp,filord,FFF,CCC,uf,yf,t,cutoff,nc,red_meth);
...
ncx=min(1,nc);
thx=zeros((ninp+2+n_nan)*max(filord)+ninp*(1-ncx),noutp);
...
for i=1:noutp
    ...
    cy=eye(n,n); zq=q*0;
    n_th=(ninp+2+n_nan)*n+ninp*(1-ncx);
    ...
    i_xp=ninp*n+1:ninp*n+n;        LW(i_xp)=LW(i_xp)*0+cutoff(2);
    if nc==0,LW(ninp*n+n+ik)=1;end
    ...
    for i_nan=1:n_nan+1
        ...
        www(ind_nan2,ninp*n+1:ninp*n+n)=lsim4(f,q,cy,zq,yf(ind_nan,i),t(ind_nan)-t(ind_nan(1)));
        w0=lsim4(f,0*q,cy,zq,0*t(ind_nan),t(ind_nan)-t(ind_nan(1)),q);
        if nc~=0
            nan_loc=(i_nan-1)*n;
            www(ind_nan2,1+(ninp+1)*n+nan_loc:(ninp+1)*n+nan_loc+n)=w0;
        else
            www(ind_nan2,1+(ninp+1)*n:(ninp+1)*n+ninp)=uf(ind_nan,:);
